...
function classCode(header: string): string {
...
function displaySubject(subjectRaw: string, config: Sp143Config): string {
...
export function lessonsForGroup(
  block: ClassBlock,
  group: Group,
  config: Sp143Config = sp143Config,
): Lesson[] {
  ...
  for (const slot of block.slots) {
    for (const weekday of block.weekdays) {
      ...
      lessons.push({
        ...
        subject: displaySubject(chosen.subjectRaw, config),
        subjectRaw: chosen.subjectRaw,
        teacher: chosen.teacher,
        room: chosen.room,
        roomLabel: displayRoom(chosen.room, config),
...
export async function parseWorkbook(
  ...
  for (let row = 0; row < grid.length; row++) {
    ...
    classes.push(parseClassBlock(grid, row, text, config));
  }
